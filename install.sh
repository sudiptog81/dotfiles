...
dotfiles_xdg_config_dir=(.alacritty .dunst .htop .i3 .i3blocks .picom .rofi .nvim)
...
install_dotfiles() {
    ...
    env rm -rf "$HOME/.oh-my-zsh"
    curl -Lo "$HOME/install.sh" "https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh"
    sh "$HOME/install.sh"
    env rm -rf "$HOME/.zshrc"
    env ln -fs "$dotfiles_repo_dir/.zshrc" "$HOME/.zshrc"
    cp -r "$dotfiles_repo_dir/.oh-my-zsh/plugins/archive" "$HOME/.oh-my-zsh/plugins/"
    cp -r "$dotfiles_repo_dir/.oh-my-zsh/themes/oxide.zsh-theme" "$HOME/.oh-my-zsh/themes/"
    env rm -rf "$HOME/install.sh"
    mkdir -p "$HOME/.local/bin"
    env ln -fs "$dotfiles_repo_dir/bin/rofi-calendar" "$HOME/.local/bin/rofi-calendar"
